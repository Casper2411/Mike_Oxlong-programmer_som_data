...
void printHeapColors() {
    printf("Heap Colors:\n");
    
    word* heapPtr = heap;
    while (heapPtr < afterHeap) {
        word header = heapPtr[0];
        printf("Block at heap[%ld]: Color = ", (long)(heapPtr - heap));
        printColor(header);
        heapPtr += Length(header) + 1;
    }
}

void recurseMark(word curr){
  if (!IsInt(curr)) {
    word* ref = (word*)curr;
    
    if(!inHeap(ref)) return;
